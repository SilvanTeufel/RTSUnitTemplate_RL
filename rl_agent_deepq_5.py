...
def extract_reward(previous_game_state, current_game_state, unreal_action_str):
    ...
    if previous_game_state:
        ...
        # --- Additional Reward: Proximity to friendly and enemy units (1 / distance) ---
        # Retrieve positions (expecting lists [x, y, z])
        agent_pos = current_game_state.get("AgentPosition", [0.0, 0.0, 0.0])
        friendly_pos = current_game_state.get("AvgFriendlyPos", [0.0, 0.0, 0.0])
        enemy_pos = current_game_state.get("AvgEnemyPos", [0.0, 0.0, 0.0])
    ...
    while episode_number < NUM_EPISODES:
        ...
        if bNewGameStateAvailable and game_state_str:
            ...
            try:
                ...
                json_end_index = find_json_end(game_state_str)
                if json_end_index != -1:
                    potential_json = game_state_str[:json_end_index + 1]
                    print(f"[RL Agent] Potential JSON (length {len(potential_json)}): {potential_json}")
                    try:
                        game_state_dict = json.loads(potential_json)
                        print("[RL Agent] Successfully parsed after truncating to the first valid JSON end.")
                        parsed_successfully = True
                    except json.JSONDecodeError as e2:
                        print(f"[RL Agent] Still error after truncation: {e2}")
                        print(f"[RL Agent] Potential JSON: {potential_json}")
                else:
                    print("[RL Agent] Could not find the end of the JSON object.")
