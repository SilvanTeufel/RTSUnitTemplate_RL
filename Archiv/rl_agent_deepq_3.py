...
STATE_SPACE_SIZE = 21  # MyUnits, EnemyUnits, MyHealth, EnemyHealth, MyAttack, EnemyAttack, AgentPosX, AgentPosY, AgentPosZ, AvgFriendlyPosX, AvgFriendlyPosY, AvgFriendlyPosZ, AvgEnemyPosX, AvgEnemyPosY, AvgEnemyPosZ, PrimaryResource, SecondaryResource, TertiaryResource, RareResource, EpicResource, LegendaryResource
...
if __name__ == "__main__":
    ...
    last_game_state_dict = None # To store the previous game state for reward calculation
    total_steps = 0
    episode_reward = 0  # Initialize reward for the current episod
    ...
    while episode_number < NUM_EPISODES:
        ...
        print(f" Episode: {episode_number} / {NUM_EPISODES}, Total Steps: {total_steps}, Current Episode Reward: {episode_reward:.2f}")
        if bNewGameStateAvailable and game_state_str:

            # print(f"[RL Agent] Received Game State: {game_state_str}")
            game_state_str = game_state_str.strip() # Remove leading/trailing whitespace
            game_state_str = game_state_str.replace('\x00', '') # Explicitly remove null characters
            ...
            try:
                ...
                total_steps += 1
                episode_number += 1 # Increment the episode counter here
      ...
      # Increment episode number at the end of each episode (you might need to adjust this based on your episode termination logic)
        # For now, we consider each iteration of the main loop as a step within an episode.
        # If you have a specific condition for the end of an episode (e.g., based on the game state),
        # you should increment episode_number there and reset episode_reward.
        if bNewGameStateAvailable and game_state_str: # Only consider a "step" if we received a new game state
            pass # The episode number is incremented based on the NUM_EPISODES loop

    print("[RL Agent] Training finished.")
    print("\n--- Episode Scores ---")
    print(f"Total Episodes: {NUM_EPISODES}")
    # Since the provided code doesn't have explicit episode termination from the Unreal side,
    # we are essentially running for a fixed number of steps defined by NUM_EPISODES.
    # The 'episode_reward' variable accumulates the reward over the entire training process.
    # If you want to track the score per actual game episode (which might span multiple steps),
    # you'll need to get an indication from the Unreal Engine when an episode ends.

    # For now, let's print the final accumulated reward.
